...
;-------------------------------------------------------------------------
;  Subroutine jump handlers for menu operations
;-------------------------------------------------------------------------
...
DRAW_TITLE:		LDA #>str_uboot_title1	; Print first row
  				STA V_P_STR_HI	
  				LDA #<str_uboot_title1
  				STA V_P_STR_LO
  				JSR LCD_P_STR	

  				LDA #40 				; Print second row
  				JSR LCD_SET_POS
...
APP_SER_TEST: 	LDA #%00000001			; Clear display
  				...
  				STZ S_BYTE_COUNT_HI
  				;LDA ACIA_CMD
  				;AND #%11111101 			; Bit 1 to zero -> enable IRQ
  				;STA ACIA_CMD
  				JMP SER_TEST_MAIN
...
SER_TEST_MAIN:	;LDA S_DATA_RDY 		
				;BEQ SER_TEST_MAIN 		
				LDA ACIA_STATUS 		; Is new data available?
				AND #%00001000
...
str_uboot_title2:
	db "Version 0.1.2",255

str_credits:
	db "Juha Koljonen                            Silicon Valley",255
...
TBL_JUMP: 		
	.word RESET, RESET, RESET, GOTO_BMENU				; Main menu items
